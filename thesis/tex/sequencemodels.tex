...
We now go on to investigate more advanced neural architectures, \acp{CNN} and
\acp{RNN}. Both are widely used in various \ac{NLP} tasks.
[...]\todo{todo}.

These architectures take the ordering of tokens into account. In \ac{NLP},
\acp{CNN} can be considered as a \ngram detector, which looks at a window of
tokens \autocite[155]{goldbergNLP}. However, unlike the count vector approach
used above, the CNN extracts `soft' \ngrams, utilizing the ability of word
embeddings to share statistical strength between words, and outputting a
real-valued number instead of a boolean yes/no value or an integer count.
...
\ac{RNN} models. These aspects are described in this section.
...
document lengths, which turned out to be 701 tokens. Then, we computed the
value $Q_2 + 1.5 \cdot (Q_3 - Q_1)$, or the median plus 1.5 times the
interquartile range, which gives 693 tokens. Finally, we computed the mean
value plus two standard deviations, giving 707 tokens. These values are all
close to each other, and we decided to settle on 700 tokens since it is a
round number close to all values we examined.
...
receive vectors of size 110. \todo{implementation details, keras etc}
...
\todo{table 5.1 presents ... we observe that ...}
