...
micro \FI scores the \ac{MLP} models in the previous chapter achieved. However,
the confusion matrix better approximates a diagonal, with gradual dropoff as
the distance from the diagonal increases \todo{is this quantifiable (MAE)}.
...
\textcite{taghipour16}. Changes to their architecture were made in order to
accommodate our data \todo{which?}. The embedding layer in
\textcite{taghipour16} was of dimension 50 and randomly initialized. We
increased the embedding dimension to 100, and experiment both with randomly
initialized embeddings and with pre-trained ones.
