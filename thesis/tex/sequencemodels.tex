...
The authors of the reference study did not investigate different prediction
methods apart from regression. A reason for this is that the ASAP dataset
which they use consists of essays from eight different prompts. Both the
range of scores and the number of possible scores differs across
these prompts. The number of possible scores is as low as 4 or as high as
61 depending on the prompt. Modelling the task as a regression allowed them
to use the same type of output layer for all the prompts, despite the
differences in scoring, by normalizing them to a common interval respective
to the prompt before training.
